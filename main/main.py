...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:@localhost/SmartPlatees'
...
with app.app_context():
    db.create_all()  # This will create all the tables (including the 'User' table)
...
symptoms_dict = {
    ...
    'burning_micturition': 12, 'spotting_urination': 13, 'fatigue': 14, 'weight_gain': 15, 'anxiety': 16, 'cold_hands_and_feets': 17,
    ...
    'loss_of_smell': 88, 'bladder_discomfort': 89, 'foul_smell_of_urine': 90, 'continuous_feel_of_urine': 91, 'passage_of_gases': 92,
    'internal_itching': 93, 'toxic_look_(typhos)': 94, 'depression': 95, 'irritability': 96, 'muscle_pain': 97, 'altered_sensorium': 98,
    'red_spots_over_body': 99, 'belly_pain': 100, 'abnormal_menstruation': 101, 'dischromic_patches': 102, 'watering_from_eyes': 103,
...
diseases_list = {
    ...
    5: 'Arthritis', 0: 'Paroxysmal Positional Vertigo', 2: 'Acne', 38: 'Urinary tract infection', 35: 'Psoriasis', 27: 'Impetigo'
}
...
def home():
    ...
    if request.method == 'POST':
        ...
        if len(symptoms) == 0:
            return render_template('index.html', prediction='Please select at least one symptom.')

        predicted_disease = get_predicted_value(symptoms)
        description, precautions, medications, diet, workout = helper(predicted_disease)

        return render_template('index.html', prediction=predicted_disease, description=description, precautions=precautions, medications=medications, diet=diet, workout=workout)
